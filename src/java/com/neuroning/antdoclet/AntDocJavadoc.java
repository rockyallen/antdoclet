...
public class AntDocJavadoc extends AntDoc {
  ...
  // recognized tags
  private static final String TAG_TASK = "ant.task";
  private static final String TAG_TYPE = "ant.type";
  private static final String TAG_NAME = "ant.name";
  private static final String TAG_REQUIRED = "ant.required";
  private static final String NOT_REQUIRED = "ant.not-required";
  private static final String IGNORE = "ignore";

  // types and attributes have the same data
  private final Map<String, AttributeData> attributes = new HashMap<String, AttributeData>();
  private final Map<String, AttributeData> nestedTypes = new HashMap<String, AttributeData>();
   ...
   */
  public static AntDocJavadoc getInstance(String clazz, RootDoc rootdoc) {
    ...
    try {
      ...
      try {
        c = Class.forName(clazz);
      } catch (ClassNotFoundException ee2) {
        System.out.println("WARNING: AntDoclet couldn't find '" + clazz + "'. Make sure it's in the CLASSPATH");
      }
    }
    if (c == null) {
      return null;
   ...
   */
  private static AntDocJavadoc getInstance(Class<? extends Object> clazz, RootDoc rootdoc) {
    Util.notNull(clazz, "clazz");
    Util.notNull(rootdoc, "rootdoc");
    ...
    if (!isIgnored(doc)) {
      ...
      for (String attributeName : getAttributes2(ih)) {
        parseAttribute(doc, attributeName, ih, d);
      }

      // parse nested types
      for (Method m : ih.getExtensionPoints()) {
        parseExtensionPoints(m, doc, d);
      }
    }
    return d;
  }

  private static void parseExtensionPoints(Method m, ClassDoc doc1, AntDocJavadoc d) throws RuntimeException, IllegalArgumentException {
    String nestedClassname = m.getParameters()[0].getType().getName();
    MethodDoc mm = getMethodForNestedType(doc1, nestedClassname);
    ...
    String s = Util.tagValue(mm, TAG_REQUIRED);
    if (s != null) {
      att.setDefaultValue(null);
      att.setRequiredComment(s);
    } else {
      s = Util.tagValue(mm, NOT_REQUIRED);
      if (s != null) {
        att.setDefaultValue("?");
        att.setNotRequiredComment(s);
      } else {
        att.setDefaultValue("?");
        att.setRequiredComment("?");
        att.setNotRequiredComment("?");
      }
    }
    d.nestedTypes.put(name, att);
  }

  private static void parseAttribute(ClassDoc doc1, String attributeName, IntrospectionHelper ih, AntDocJavadoc d) throws RuntimeException {
    MethodDoc mm = getMethodFor(doc1, attributeName);
    if (mm != null) {
      Util.notNull(mm, attributeName + " on " + doc1);
      AttributeData att = new AttributeData();
      ...
      if (Util.tagValue(mm, TAG_REQUIRED) != null) {
        att.setDefaultValue(null);
        att.setRequiredComment(Util.tagValue(mm, TAG_REQUIRED));
      } else if (Util.tagValue(mm, NOT_REQUIRED) != null) {
        att.setDefaultValue("");
        att.setNotRequiredComment(Util.tagValue(mm, NOT_REQUIRED));
      } else {
        att.setDefaultValue("?");
        att.setRequiredComment(null);
        att.setNotRequiredComment(null);
      }
      d.attributes.put(attributeName, att);
    }
  }

  private static Collection<String> getAttributes2(IntrospectionHelper introHelper) {
  ...
  /**
   * @return Whether this represents an Ant Task (otherwise, it is assumed as a
   * Type)
   */
  @Override
  ...
  /**
   * To be ignored you must be tagged first and have @ignore set to true.
   *
   * If you are not tagged, you should not be ignored.
   *
   * @param doc
   * @return
   */
  private static boolean isIgnored(Doc doc) {
    return Util.tagValue(doc, TAG_TASK) != null && "true".equalsIgnoreCase(Util.tagAttributeValue(doc, TAG_TASK, IGNORE))
            || Util.tagValue(doc, TAG_TYPE) != null && "true".equalsIgnoreCase(Util.tagAttributeValue(doc, TAG_TYPE, IGNORE));
  }
  ...
  public boolean sourceIncluded() {
    boolean ret = doc.isIncluded();
    if (!ret) {
      System.out.println("WARNING: Doc missing for class: " + this);
    }
    return ret;
  }
  ...
  /**
   * Returned the nested element names.
   *
...
//    Enumeration<String> ret = introHelper.getNestedElements();
//    while (ret.hasMoreElements()) System.out.println("Nested: "+ret.nextElement());
  ...
  public Iterator<String> getNestedTypes() {
    return nestedTypes.keySet().iterator();
  }
   ...
   */
  public Class<? extends Object> getElementType(String elementName) {
    Util.notNull(elementName, elementName);
    Class<? extends Object> ret = introHelper.getElementType(elementName);
    Util.notNull(ret, elementName);
  ...
  public AntDoc getElementDoc(String elementName) {
    Util.notNull(elementName, "elementName");
    Class<? extends Object> type = getElementType(elementName);
    Util.notNull(type, "for " + elementName);
    AntDoc ret = getInstance(type, rootdoc);
    Util.notNull(ret, elementName);
    return ret;
  }

  /**
   * Return a new AntDoc for the "container" of this type. Only makes sense for
   * inner classes.
   *
   */
  public AntDoc getContainerDoc() {
    ...
    return getInstance(doc.containingClass().qualifiedName(), rootdoc);
  }
  ...
  /**
   * Get the comment about the requirement of this attribute. The comment is
  ...
  public String getAttributeRequired(String attribute) {
    return attributes.get(attribute).getRequiredComment();
  }

  /**
   * Get the comment about the non-requirement of this attribute.
   *
   * The comment if extracted from the ant.not-required tag
   *
   * @param attribute
   * @return A comment. A null String if this attribute is not declared as
   * non-required
  ...
  public String getAttributeNotRequired(String attribute) {
    return attributes.get(attribute).getNotRequiredComment();
  }

  /**
   * Return the "category" of this Ant "task" or "type"
   *
   * @returns The value of the "category" attribute of the ant.task or ant.type
   * if it exists.
   */
  @Override
  public String getAntCategory() {

    String antCategory = Util.tagAttributeValue(doc, TAG_TASK,
            "category");

    if (antCategory == null) {
      antCategory = Util.tagAttributeValue(doc, TAG_TYPE,
              "category");
    }
  ...
  public boolean isTagged() {
    return Util.tagAttributeValue(doc, TAG_TASK, TAG_NAME) != null
            || Util.tagAttributeValue(doc, TAG_TYPE, TAG_NAME) != null;
  }
  ...
  public String getAntName() {
    String antName = Util.tagAttributeValue(doc, TAG_TASK, TAG_NAME);

    if (antName == null) {
      antName = Util.tagAttributeValue(doc, TAG_TYPE, TAG_NAME);
    }
  ...
  public String getAttributeComment(String attributeName) {
    return attributes.get(attributeName).getComment();
  }

  /**
   * Searches the given class for the appropriate setter or getter for the given
   * attribute. This method only returns the getter if no setter is available.
  ...
  private static MethodDoc getMethodFor(ClassDoc classDoc, String attribute) {
    ...
    for (MethodDoc m : methods) {
      if (m.name().equalsIgnoreCase("set" + attribute)) {
        return m;
      } else if (m.name().equalsIgnoreCase("get" + attribute)) {
        result = m;
      }
    }
  ...
  /**
   * Get the addConfigured(nestedClassname) method.
   *
   * Tries superclasses if there is nothing in this one.
   *
   * BUG? ant.getExtensionPoints returns package.Outer$Inner s as the class
   * name. In 1.8 (I haven't tried other versions) methoddoc returns
   * package.Outer.Inner. This causes this method to not find the method for
   * inner classes. Have hacked it by changing dollars to dots, but is this
   * robust?
   *
   * @param classDoc
  ...
  private static MethodDoc getMethodForNestedType(ClassDoc classDoc, String nestedClassname) {
    ...
    for (MethodDoc m : classDoc.methods()) {
      if (m.name().equalsIgnoreCase("addConfigured") || m.name().equalsIgnoreCase("add")) {
        String paramClassName = m.parameters()[0].type().qualifiedTypeName();
        if (nestedClassname.replace("$", ".").equals(paramClassName.replace("$", "."))) {
          result = m;
          break;
        }
      }
    ...
    return Util.notNull(result, nestedClassname);
  }
  ...
  private static String typeToString(Class<? extends Object> clazz) {
    ...
    if (EnumeratedAttribute.class.isAssignableFrom(clazz)) {
      try {
        ...
        // ignore, may be a protected/private Enumeration
        // iae.printStackTrace();
